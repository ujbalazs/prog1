...
class TicTacToe(Game):
    ...
    def __init__(self, h=4, v=4, k=4):
        ...
        self.v = v
        self.k = k
        steps = [(x, y, c) for x in range(1, h + 1) for y in range(1, v + 1) for c in ('p','k','z','s')]
        print(steps)
        print(type(steps))
        print(steps[0])
        self.initial = {'next': 'X',
    ...
    def legal_steps(self, state):
        ...
        neighbor_colors = [state["board"].get((x, y)) for x in (-1,0,1) for y in (-1,0,1) if (x,y) != (0,0)]
        return [s for s in state['steps'] if s[2] not in neighbor_colors and Counter(state["board"].values())[s[2]] < 3]
    ...
    def result(self, board, step, player):
        ...
        if (self.check_quads(board, step, player, (0, 1)) or self.check_quads(board, step, player, (1, 0)) or
                self.check_quads(board, step, player, (1, -1)) or self.check_quads(board, step, player, (1, 1))):
            return 1 if player == 'X' else -1
        return 0

    def check_quads(self, board, step, player, direction):
        """Check for quads in a direction."""
        delta_x, delta_y = direction
        x, y = step[:2]
        n = 0
        colors = set()
        while board.get((x, y)) == player and board.get((x, y)) not in colors:
